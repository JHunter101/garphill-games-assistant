...
const actionData: { [key: string]: { [key: string]: action[] } } = {
  'West Kingdom, Architects of the': {
    ...
    2: [
      ...
      {
        text: '<b>Towncenter</b>. Target the location with the <b>largest group of workers of a single color</b>. Perform the <b>Capture</b> action on the <b>x largest groups of automaton workers of a single color</b>. X is the amount of automaton workers on the Towncenter.',
        weight: 0.5,
        dev: 0,
      },
      {
...
type ActionTable = { [key: number]: action[] };
...
function setup(): void {
  ...
  const gameActionTables: { [key: string]: ActionTable } = {};
